...
export async function POST(request) {
  try {
    ...
    const { data: subscription, error: subError } = await supabase
      ...
      .eq('host_id', host.id)
      .single();
    if (subError || !subscription) return Response.json({ error: 'Subscription not found.' }, { status: 404 });
    if (subscription.status === 'active' && subscription.razorpay_payment_id) {
      return Response.json({ success: true, alreadyVerified: true });
    }
    if (subscription.razorpay_order_id !== razorpay_order_id) {
      return Response.json({ error: 'Order does not match this subscription.' }, { status: 400 });
    }
    ...
    if (!paymentResponse.ok) return Response.json({ error: payment?.error?.description || 'Unable to verify Razorpay payment.' }, { status: 500 });
    ...
    if (Number(payment.amount) !== Math.round(Number(subscription.total_amount) * 100)) {
      return Response.json({ error: 'Paid amount does not match the subscription amount.' }, { status: 400 });
    }
    ...
    const currentExpiry = activeRows?.[0]?.expires_at ? new Date(activeRows[0].expires_at) : null;
    const startsAt = currentExpiry && currentExpiry > now ? currentExpiry : now;
    const expiresAt = addOneMonth(startsAt);
    const paidAt = payment.created_at ? new Date(Number(payment.created_at) * 1000) : now;
    ...
    const { error: settingError } = await supabase
      ...
      .upsert({ property_id: subscription.property_id, subscription_required: true, updated_at: new Date().toISOString() }, { onConflict: 'property_id' });
    if (settingError) throw settingError;

    return Response.json({ success: true, startsAt: startsAt.toISOString(), expiresAt: expiresAt.toISOString() });
  } catch (error) {
    const message = error?.message || 'Unable to verify subscription payment.';
