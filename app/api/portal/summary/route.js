...
export async function GET(req){
  try{
    ...
    if(portal==='admin'){
      ...
      const [{count:notif},{count:pendingHosts},{count:pendingVerifications}]=await Promise.all([
        ...
        db.from('identity_verifications').select('id',{count:'exact',head:true}).eq('verification_status','pending')
      ]);
      return Response.json({success:true,messages:0,notifications:notif||0,hosts:pendingHosts||0,verifications:pendingVerifications||0,bookings:0});
    }
