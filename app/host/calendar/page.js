...
function dateInStay(day, checkIn, checkOut) { if(!checkIn||!checkOut)return false; const d=typeof day==='string'?parseDate(day):day; return d >= parseDate(checkIn) && d < parseDate(checkOut); }
function dateInBlock(day, start, end) { if(!start||!end)return false; const d=typeof day==='string'?parseDate(day):day; return d >= parseDate(start) && d <= parseDate(end); }
