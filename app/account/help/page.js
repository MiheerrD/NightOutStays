...
export default function GuestHelpPage() {
  ...
  async function loadGuest(session) {
    ...
    guestRow = byUser?.[0] || null;

    if (!guestRow && user.email) {
  ...
  async function load(preferredTicketId = "") {
    try {
      ...
      if (!session?.user) {
        window.location.href = "/login?redirect=/account/help";
        return;
      }

      const guestRow = await loadGuest(session);
      ...
      const response = await fetch("/api/support?requesterType=guest", {
  ...
  return (
    <>
      ...
      <main style={s.page}>
        <div style={s.heading}>
          <div>
            <h1 style={s.h1}>Guest Support</h1>
